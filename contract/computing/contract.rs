...
mod tests {
    use super::*;
    use cosmwasm_std::{from_binary, testing::*, Coin, StdError, Uint128};
    ...
    fn proper_initialization() {
        ...
        let info = mock_info(
            "creator",
            &[Coin { denom: "earth".to_string(), amount: Uint128::new(1000) }],
        );
    ...
    fn increment() {
        ...
        let info =
            mock_info("creator", &[Coin { denom: "token".to_string(), amount: Uint128::new(2) }]);
        let init_msg = InstantiateMsg { count: 17 };
        ...
        let info =
            mock_info("anyone", &[Coin { denom: "token".to_string(), amount: Uint128::new(2) }]);
